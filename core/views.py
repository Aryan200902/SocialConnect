...
def index(request):
    ...
    new_suggetions_list = [x for x in list(all_users) if (x not in list(user_following_all))]
    current_user = User.objects.filter(username=request.user.username)
    final_suggesting_list = [x for x in list(new_suggetions_list) if (x not in list(current_user))]
    random.shuffle(final_suggesting_list)

    username_profile = []
    username_profile_list = []
    
    for users in final_suggesting_list:
